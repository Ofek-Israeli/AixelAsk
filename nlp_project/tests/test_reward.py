...
class TestBasicReward:
    ...
    def test_basic_weighted_reward(self):
        """r = 1.0*1 + 0.5*1 - 0.1*(3/10) - 0.5*0 = 1.47."""
        cfg = _StubConfig()
        r = compute(r_correct=1.0, r_valid=1.0, depth=3, is_invalid=False, config=cfg)
        assert r == pytest.approx(1.47)

    def test_invalid_dag_penalty(self):
        """r_valid=0, is_invalid=True → includes -w_invalid, depth_term zeroed."""
        cfg = _StubConfig()
        r = compute(r_correct=0.0, r_valid=0.0, depth=3, is_invalid=True, config=cfg)
        # r = 1.0*0 + 0.5*0 - 0.1*0 (depth zeroed) - 0.5*1 = -0.5
        assert r == pytest.approx(-0.5)

    def test_parse_failure_max_penalty(self):
        """Unparseable → r_correct=0, r_valid=0, is_invalid=True."""
        cfg = _StubConfig()
        r = compute(r_correct=0.0, r_valid=0.0, depth=0, is_invalid=True, config=cfg)
        assert r == pytest.approx(-0.5)
...
class TestDepthNormalization:
    ...
    def test_none_mode(self):
        ...
        r = compute(r_correct=1.0, r_valid=1.0, depth=5, is_invalid=False, config=cfg)
        # 1.0*1 + 0.5*1 - 0.1*5 - 0 = 1.5 - 0.5 = 1.0
    ...
    def test_divide_by_max_depth_clamped(self):
        ...
        cfg = _StubConfig(REWARD_DEPTH_NORMALIZATION="DIVIDE_BY_MAX_DEPTH", REWARD_MAX_DEPTH=10)
        r = compute(r_correct=1.0, r_valid=1.0, depth=15, is_invalid=False, config=cfg)
        # 1.0 + 0.5 - 0.1*1.0 = 1.4
...
class TestPartialCredit:
    ...
    def test_exact_match(self):
        ...
        cfg = _StubConfig()
        r = compute(r_correct=1.0, r_valid=1.0, depth=1, is_invalid=False, config=cfg)
        assert r > 0

    def test_no_match(self):
        """No credit: r_correct=0."""
        cfg = _StubConfig()
        r = compute(r_correct=0.0, r_valid=1.0, depth=1, is_invalid=False, config=cfg)
        # 0 + 0.5 - 0.1*(1/10) = 0.49
    ...
    def test_partial_credit_in_range(self):
        ...
        cfg = _StubConfig()
        r = compute(r_correct=ratio, r_valid=1.0, depth=1, is_invalid=False, config=cfg)
        r_full = compute(r_correct=1.0, r_valid=1.0, depth=1, is_invalid=False, config=cfg)
        r_zero = compute(r_correct=0.0, r_valid=1.0, depth=1, is_invalid=False, config=cfg)
...
class TestEdgeCases:
    ...
    def test_zero_weights(self):
        ...
        cfg = _StubConfig(
            ...
            REWARD_WEIGHT_INVALID_PENALTY=0.0,
        )
        r = compute(r_correct=1.0, r_valid=1.0, depth=10, is_invalid=True, config=cfg)
        assert r == pytest.approx(0.0)

    def test_determinism(self):
        """Same inputs → same reward (no stochastic component)."""
        cfg = _StubConfig()
        r1 = compute(r_correct=0.8, r_valid=1.0, depth=4, is_invalid=False, config=cfg)
        r2 = compute(r_correct=0.8, r_valid=1.0, depth=4, is_invalid=False, config=cfg)
        assert r1 == r2
